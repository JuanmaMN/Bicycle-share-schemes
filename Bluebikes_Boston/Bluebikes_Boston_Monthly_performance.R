...
second_graph  <- data_source %>% 
  ...
  scale_y_continuous(limit = c(0, 3250000), expand=c(0,1)) +
  guides(fill = NULL) +
  theme(
    axis.title.x =  element_blank(),
    axis.title.y =  element_blank(),
    ...
    strip.background = element_blank(),
    strip.text.x = element_blank()
  )+annotate("text", x = 2016, y = 1800000, hjust = 0, vjust = 1,fontface=3, size = 3, colour = "#22292F",
             family = font_labels, label =  str_wrap("Note: This data has been processed to remove trips that 
                                                                              ...
                                                                              In 2018, Blue Cross Blue Shield of Massachusetts became the title sponsor for 
                                                                              the system and the name was changed to Bluebikes."))
...
patchworkBoston <- first_graph | second_graph


PBoston <-patchworkBoston + plot_annotation(title = "Bluebikes Boston",
                                       subtitle = "Metro Boston's public bike share program - monthly trip numbers",
